...
def run_metric_calculator(file):
    ...
    #G.add_nodes_from([(dict(d)["protein1"], {"protein_freq":dict(d)["rpkm_x"], "protein_name":dict(d)["protein1_name"]})
    #                      for n, d in network_df.iterrows()])
    #G.add_nodes_from([(dict(d)["protein2"], {"protein_freq":dict(d)["rpkm_y"], "protein_name":dict(d)["protein2_name"]})
    #                      for n, d in network_df.iterrows()])
    ...
    metrics_df.to_csv("metrics_data/drosophila_m/network_metrics_"+file[:-4]+".csv")
    degree_centrality.to_csv("metrics_data/drosophila_m/degree_centralities_"+file[:-4]+".csv")
    node_degrees.to_csv("metrics_data/drosophila_m/degrees_"+file[:-4]+".csv")
    clust_df.to_csv("metrics_data/drosophila_m/clustering_coefs_"+file[:-4]+".csv")
    node_bc_df.to_csv("metrics_data/drosophila_m/node_bcs_"+file[:-4]+".csv")
    print(file)
...
files_transcriptomics = []
for file in files:
    if "protein" not in file:
        files_transcriptomics.append(file)
          
with Pool(processes=5) as pool:
    pool.map(run_metric_calculator, files_transcriptomics)